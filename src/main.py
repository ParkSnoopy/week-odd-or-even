...
if __name__ in {"__main__", "__mp_main__"}:
    if app.storage.general.get("_date_delta", None) is None:
        _date_delta = questionary.text(
            "Set Date Delta",
        ).ask()
        try:
            _date_delta = int(_date_delta)
        except:
            _date_delta = 1
        app.storage.general["_date_delta"] = _date_delta

    _date_delta = timedelta(
        days=int(
            app.storage.general["_date_delta"]
        ),
    )
